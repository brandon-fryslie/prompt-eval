...
```bash
lnks ls --query "status:open type:feature" --sort priority:asc,updated_at:asc --json
```
...
```bash
lnks start <ticket-id> --reason "starting implementation"
```
...
- **If the ticket is too large**, break it into smaller sub-tickets using `lnks new` with `lnks dep add`, then work the first sub-ticket only.
- **If you're blocked**, leave the ticket in_progress, comment with `lnks comment <id> "blocked: <reason>"`, and stop.
